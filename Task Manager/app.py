...
def mark_seen():
    ...
    if section not in ["announcements", "files", "tasks"]:
...
def has_new():
    ...
    cur.execute("""
        SELECT announcements_last_seen, files_last_seen, tasks_last_seen
        FROM users WHERE id=?
    """, (user_id,))
    user = cur.fetchone()
...
def submit_employee_approval():
    ...
    if not approval_type or not payload or not assigned_to:
        return jsonify({
            "status": "error",
