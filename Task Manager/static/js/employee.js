...
function loadLeaveApprovals() {
    fetch("/employee/approvals?type=leave")
        ...
        .catch(() => {
            console.error("Failed to load leave approvals");
        });
}
...
function renderLeaveTimeline(rows) {
    ...
    rows.forEach(r => {
        ...
        if (r.status === "Rejected") {
            statusText = "Rejected";
            extraInfo = `Rejected: ${r.payload.reason || "-"}`;
        }
    ...
    fetch("/employee/has-new")
        ...
        .then(data => {
            if (data.announcements) {
                document.getElementById("annDot").style.display = "inline-block";
            }
            if (data.files) {
                document.getElementById("fileDot").style.display = "inline-block";
            }
            if (data.tasks) {
                document.getElementById("taskBadge").style.display = "inline-block";
            }
        });
}
