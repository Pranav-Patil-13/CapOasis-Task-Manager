...
def admin_approval_action():
    ...
    # ✅ APPLY EFFECTS ONLY IF APPROVED
    if action == "approve":
        apply_approval_effects(approval)
    ...
    create_notification(
        ...
        message=f"Your request was {new_status.lower()}",
        notif_type="approval",
        actor="Admin",
        ref_type="approval",
        ref_id=approval_id
    ...
    log_activity(
        "Admin",
        f"{new_status} {approval['employee_name']}'s approval"
    )

    return jsonify({"status": "success"})

def apply_approval_effects(approval_row):
    """
    Apply side-effects AFTER approval is approved.
    Safe dispatcher: never crashes approval.
    """

    try:
        if approval_row["type"] == "regularisation":
            apply_regularisation(approval_row)

        elif approval_row["type"] == "leave":
            apply_leave(approval_row)

        elif approval_row["type"] == "reimbursement":
            apply_reimbursement(approval_row)

    except Exception as e:
        print("⚠️ Approval side-effect failed:", e)
...
def create_notification(
    ...
    cur.execute("""
        ...
        (user_id, actor_name, type, title, message, reference_type, reference_id, is_read, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?)
    """, (
        user_id,
        actor,
...
def apply_regularisation(approval_row):
    conn = get_db()
    cur = conn.cursor()

    payload = json.loads(approval_row["payload"])
    employee_id = approval_row["employee_id"]
    approval_id = approval_row["id"]

    reg_date = payload["date"]
    requested = payload["requested"]

    req_status = requested.get("status")
    req_check_in = requested.get("check_in")
    req_check_out = requested.get("check_out")
    ...
    before_snapshot = {
        "status": before_row["status"] if before_row else "Absent",
        "check_in": before_row["check_in_time"] if before_row else None,
        "check_out": before_row["check_out_time"] if before_row else None,
        "day_type": before_row["day_type"] if before_row else None,
        "late_comment": before_row["late_comment"] if before_row else None
    }
    ...
    if req_status == "HALF":
        day_type = "HALF"
    elif str(req_status).lower() == "late":
        late_comment = "late"

    after_snapshot = {
        "status": req_status,
        "check_in": req_check_in,
        "check_out": req_check_out,
        "day_type": day_type,
    ...
    if cur.fetchone():
        ...
        cur.execute("""
            ...
            (user_id, date, status, check_in_time, check_out_time, day_type, late_comment)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (
            employee_id,
            reg_date,
    ...
    # ---- OVERRIDE LEAVE (NOT APPROVAL STATUS) ----
    cur.execute("""
        SELECT id, payload
        FROM approvals
        WHERE employee_id = ?
    ...
    for lr in cur.fetchall():
        ...
        if lp.get("from_date") <= reg_date <= lp.get("to_date"):
            cur.execute("""
                UPDATE approvals
...
def apply_leave(approval_row):
    """
    Leave side-effects are already handled elsewhere.
    Approval decision is enough for now.
    """
    pass

def apply_reimbursement(approval_row):
    """
    Reimbursement side-effects are intentionally empty for now.
    Approval itself is enough.
    Accounting / payout logic will be added later.
    """
    pass
...
def employee_today_attendance():
    ...
    # ✅ Record exists but no check-in yet
...
def uploaded_file(filename):
    ...
    # ✅ ADMIN: full access to uploads (reimbursement bills etc.)
    if role == "admin":
        return send_from_directory(app.config['UPLOAD_FOLDER'], filename)

    # 🔒 EMPLOYEE: restrict to shared files only
    conn = get_db()
    cur = conn.cursor()
...
@app.route("/admin/remunerations")
def admin_get_remunerations():
    if session.get("role") != "admin":
        return jsonify({"status": "forbidden"}), 403
...
@app.route("/admin/remunerations/action", methods=["POST"])
def admin_remuneration_action():
    if session.get("role") != "admin":
        return jsonify({"status": "forbidden"}), 403
...
def employee_check_in():
    ...
    if is_on_approved_leave(session["user_id"], today):
        return jsonify({
        "status": "error",
    ...
    cur.execute("""
        ...
        CASE
            WHEN EXISTS (
                SELECT 1
                FROM approvals a
                WHERE a.employee_id = attendance.user_id
                AND a.type = 'regularisation'
                AND a.status = 'Approved'
                AND a.payload IS NOT NULL
                AND json_valid(a.payload)
                AND json_extract(a.payload, '$.date') = attendance.date
            )
            THEN 1
            ELSE 0
        END AS regularised
...
def has_new():
    ...
    ann_seen = user["announcements_last_seen"] or "1970-01-01T00:00:00+00:00"
    file_seen = user["files_last_seen"] or "1970-01-01T00:00:00+00:00"
    task_seen = user["tasks_last_seen"] or "1970-01-01T00:00:00+00:00"
...
def is_on_approved_leave(user_id, date_str):
    conn = get_db()
    cur = conn.cursor()
    ...
    cur.execute("""
        ...
        WHERE employee_id = ?
          ...
          AND status = 'Approved'
          AND ? BETWEEN
              json_extract(payload, '$.from_date')
              AND json_extract(payload, '$.to_date')
        LIMIT 1
    """, (user_id, date_str))
...
def submit_employee_approval():
    ...
    # ================= REGULARISATION SCHEMA VALIDATION =================
    if approval_type == "regularisation":

        # required keys
        required_keys = ["date", "current", "requested", "reason"]
        ...
        today = now_ist().date()
        if (today - req_date).days > 7:
            return jsonify({
                "status": "error",
                "message": "Regularisation allowed only for last 7 days"
            }), 400

    # ✅ Support BOTH old + new UI
    assigned_to = data.get("assigned_to")

    # ================= BLOCK DUPLICATE REGULARISATION =================
    if approval_type == "regularisation":
        reg_date = payload.get("date")

        if not reg_date:
            return jsonify({
                "status": "error",
                "message": "Invalid regularisation date"
            }), 400
        ...
        cur.execute("""
            ...
            WHERE
                employee_id = ?
                AND type = 'regularisation'
                AND status IN ('Pending', 'Approved')
                AND json_extract(payload, '$.date') = ?
        """, (session["user_id"], reg_date))
        ...
        if existing:
            return jsonify({
                "status": "error",
                "message": "Regularisation already submitted for this date"
            }), 400
...
@app.route("/employee/approval/<int:approval_id>")
def employee_view_approval(approval_id):
    # 1️⃣ Auth check
    if session.get("role") != "employee":
        return jsonify({"status": "forbidden"}), 403
    ...
    # 2️⃣ Fetch approval + ensure ownership
    cur.execute("""
        SELECT
            a.id,
            a.type,
...
@app.route("/employee/notifications/read", methods=["POST"])
...
@app.route("/employee/notifications/clear", methods=["POST"])
...
@app.route("/employee/reimbursement", methods=["POST"])
def submit_reimbursement():
    if session.get("role") != "employee":
        return jsonify({"status": "forbidden"}), 403

    bill = request.files["bill"]
    filename = f"reimb_{int(datetime.now().timestamp())}_{bill.filename}"
    ...
    cur.execute("""
        INSERT INTO approvals
        (type, payload, status, employee_id, assigned_to, created_at)
        VALUES (?, ?, 'Pending', ?, ?, ?)
    """, (
        "reimbursement",
        json.dumps(payload),
    ...
    create_notification(
        ...
        message=f"₹{payload['amount']} · {payload['category']}",
        notif_type="approval",
        actor="Employee"
    )
...
@app.route("/employee/remuneration", methods=["POST"])
def employee_submit_remuneration():
    if session.get("role") != "employee":
        return jsonify({"status": "forbidden"}), 403
    ...
    cur.execute("""
        ...
        (employee_id, type, amount, reason, month, requested_by, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (
        session["user_id"],
        data.get("type"),
...
@app.route("/employee/remuneration", methods=["GET"])
def employee_get_remunerations():
    if session.get("role") != "employee":
        return jsonify({"status": "forbidden"}), 403
